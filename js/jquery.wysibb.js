...
!function ( $ ) {
	...
	$.wysibb.prototype = {
		...
		init : function ( ) {
			...
			// phpbb2
			this.$txtArea.parents( 'form' ).find( 'input[id*="preview"],input[id*="submit"],input[class*="preview"],input[class*="submit"],input[name*="preview"],input[name*="submit"]' )
		...
		initTransforms : function ( ) {
			...
			this.options.btnlist = btnlist; // use for transforms, becouse select
			// elements not present in buttons
			...
			// create rootsel list
			this.rsellist = [ ];
			for ( rootsel in this.options.rules ) {
		...
		build : function ( ) {
			...
			// this.$editor.append('<span class="powered">Powered by <a
			// href="http://www.wysibb.com" target="_blank">WysiBB<a/></span>');
		...
		buildColorpicker : function ( container, bn, opt ) {
			...
			// TODO: change this from static HTML to dynamic HTML, but adding a class
			// name to designate the wrapping element for JavaScript
			var $btnHTML;
			...
			var $btn = $( '<div class="wysibb-toolbar-btn wbb-dropdown wbb-cp">' ).appendTo( container ).append( $btnHTML ).append( this
			...
			else if ( opt.colors && typeof opt.colors === 'string' ) {
				colorlist = opt.colors.split( ',' );
			}
			else {
				colorlist = [ ];
			}
		...
		buildTablepicker : function ( container, bn, opt ) {
			...
			for ( var j = 1; j <= cols; j++ ) {
				for ( var h = 1; h <= rows; h++ ) {
					// var html = this.strf('<div class="tbl-sel"
		...
		initModal : function ( ) {
			...
			if ( this.$modal.size() === 0 ) {
				...
				$( document ).bind( 'keydown', $.proxy( this.escModal, this ) ); // ESC
				// key
				// close
				// modal
			}
			...
			if ( this.options.bbmode ) {
				...
				if ( opt.excmd ) {
					...
					if ( withvalue ) {
						...
						try { // Firefox fix, exception while get queryState for
							// UnorderedList
							if ( ( opt.excmd === 'bold' || opt.excmd === 'italic' || opt.excmd === 'underline' || opt.excmd === 'strikeThrough' ) && $( this.getSelectNode() ).is( 'img' ) ) { // Fix,
								// when
								// img
								// selected
								return false;
		...
		wbbExecCommand : function ( command, value, queryState ) { // default
			// command for
			// custom
			// bbcodes
		...
		wbbRemoveCallback : function ( command, clear ) {
			...
			if ( this.options.bbmode ) {
				...
				$.each( opt.rootSelector, $.proxy( function ( i, s ) {
					...
					if ( $root.is( 'span[wbb]' ) || !$root.is( 'span,font' ) ) { // remove
						// only
						// blocks
						if ( clear === true || ( !cs || !cs[ 'seltext' ] ) ) {
							...
							if ( cs && cs[ 'seltext' ] && cs[ 'seltext' ][ 'sel' ] ) {
								htmldata = $root.find( cs[ 'seltext' ][ 'sel' ] ).html();
		...
		execNativeCommand : function ( cmd, param ) {
			...
			if ( cmd === 'insertHTML' && !window.getSelection ) { // IE does't support
				...
				// fix
				r.pasteHTML( param );
				var txt = $( '<div>' ).html( param ).text(); // for ie selection inside
				// block
		...
		saveRange : function ( ) {
			this.setBodyFocus();
			// this.lastRange=(this.options.bbmode) ?
			// this.getCursorPosBB():this.getRangeClone();
		...
		getHTML : function ( bbdata, init, skiplt ) {
			 ...
			 * $.each(this.options.smileList,$.proxy(function(i,row) { var fidx =
			 * ndata.indexOf(row.bbcode); if (fidx!=-1) { var afternode_txt =
			 * ndata.substring(fidx+row.bbcode.length,ndata.length); var afternode =
			 * document.createTextNode(afternode_txt); el.data = ndata =
			 * el.data.substr(0,fidx);
			 * $(el).after(afternode).after(this.strf(row.img,this.options)); }
			 * },this)); }
			 */
			this.getHTMLSmiles( $wrap );
			// $wrap.contents().filter(function() {return
			// this.nodeType==3}).each($.proxy(this,smileRPL,this));
		...
		clearFromSubInsert : function ( html, cmd ) {
			if ( this.options.allButtons[ cmd ] && this.options.allButtons[ cmd ].rootSelector ) {
				...
				$.each( this.options.allButtons[ cmd ].rootSelector, $.proxy( function ( i, s ) {
					...
					var res = true;
					$wr.find( '*' ).each( function ( ) { // work with find("*") and "is",
						// becouse in ie7-8 find is case
						// sensitive
						if ( $( this ).is( s ) ) {
		...
		clearPaste : function ( el ) {
			 ...
			 * $block.find(s).attr("wbbkeep",1); },this));
			 * 
			 * //replace div and p without last br to html()+br
			 * $block.find("*[wbbkeep!='1']").each($.proxy(function(i,el) { var $this =
			 * $(el); if ($this.is('div,p') && ($this.children().size()==0 ||
			 * el.lastChild.tagName!="BR")) { $this.after("<br/>").after($this.contents()).remove();
			 * }else{ $this.after($this.contents()).remove(); } },this));
			 * $block.find("*[wbbkeep]").removeAttr("wbbkeep").removeAttr("style");
			 */
		},
		...
		showModal : function ( cmd, opt, queryState ) {
											...
											.each( r.input, $
													.proxy( function ( j, inp ) {
														...
														if ( inp.type && inp.type === 'div' ) {
															...
															$c
																	.append( this
																			.strf( '<div class="wbbm-inp-row"><label>{title}</label><div class="inp-text div-modal-text" contenteditable="true" name="{param}">{value}</div></div>', inp ) );
														}
														else {
															// default input
															$c
																	.append( this
																			.strf( '<div class="wbbm-inp-row"><label>{title}</label><input class="inp-text modal-text" type="text" name="{param}" value="{value}"/></div>', inp ) );
														}
			...
			$( document.body ).css( 'overflow', 'hidden' ); // lock the screen, remove
			// scroll on body
			if ( $( 'body' ).height() > $( window ).height() ) { // if body has
				// scroll, add
				// padding-right
				// 18px
			...
			// setTimeout($.proxy(function()
			// {this.$modal.find("input:visible")[0].focus()},this),10);
			setTimeout( $.proxy( function ( ) {
		...
		closeModal : function ( ) {
			$( document.body ).css( 'overflow', 'auto' ).css( 'padding-right', '0' ).unbind( 'keyup', this.escModal ); // ESC
			// key
			// close
			// modal;
